...
class ReportKerusakanController extends Controller {
    public function __construct(){
        $this->middleware(['auth']);
    }

    public function index() {
        return view('backend.report.kerusakan');
    }
    ...
    public function report(Request $request) {
        ...
        if ($rules) {
            if($request->date_start < $request->date_end) {
                ...
                $pdf = PDF::loadView('backend.report.kerusakanPrintReport', compact(['data','date','date_from','date_to']));
                return $pdf->stream();
            } else {
                return redirect('/manage/report-kerusakan')->with('failed', 'Tanggal Awal Harus Lebih Kecil');
            }
        }
    }
